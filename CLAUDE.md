...
- **Keep-Office/keep-office** — this distribution repo (deploy scripts, overlays, patches). Old `ritza-co/work-eu` redirects here.
- **Keep-Office/keep-office-portal** — detached public fork of `MinBZK/bureaublad` @ `v0.9.3`. We own the portal now; brand/UI changes go here directly (not as patches). Already rebranded to "Keep Office".
...
- Docker dev workflow lives on the VPS (many `localhost/bureaublad-frontend:<tag>` images). Current Keep Office patch image: `localhost/bureaublad-frontend:keepoffice-1`.
...
- Patch live via kcadm (admin pw in secret `keycloak-keycloak` key `admin-password`, but the working creds were the **bootstrap** file `$KC_BOOTSTRAP_ADMIN_PASSWORD_FILE`, user `admin`). kcadm's `/` HOME is read-only → must pass `--config /tmp/kc.config`:
  ...
  kubectl -n mb-keycloak exec keycloak-keycloak-0 -c keycloak -- sh -c '
   ...
   $KC update realms/mijnbureau --config $CFG -s "displayName=Keep Office" -s "displayNameHtml=<b>Keep Office</b>"'
  ```
...
## Brand strings (in keep-office-portal)
`frontend/messages/{en,nl}.json` → `HomePage.title` (header), `HomePage.welcome`, `Footer.copyright`; `frontend/src/app/layout.jsx` → tab `<title>`. All set to "Keep Office".
